...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ULMEXParams"
    else
        PARAMS_DIR="$HOME/.ulmex-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
ULMEX - install-params.sh

This script will install the ULMEX zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ULMEX zkSNARK parameters. Note that it is
