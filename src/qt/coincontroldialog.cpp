...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select ULME Outputs to Spend" :
            "Select Shielded ULME to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::ULME;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show ULMEX address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
