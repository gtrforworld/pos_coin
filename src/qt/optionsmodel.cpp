...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ULME);
    ...
    if (settings.contains("nAnonymizeUlmexAmount") || reset)
        gArgs.SoftSetArg("-anonymizeulmexamount", settings.value("nAnonymizeUlmexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ulmex.cpp during shutdown to also remove the window positions
