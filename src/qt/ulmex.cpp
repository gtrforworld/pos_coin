...
#include "qt/ulmex/splash.h"
#include "qt/ulmex/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/ulmex/governancemodel.h"
...
{
    return QCoreApplication::translate("ulmex-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ulmex.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ulmex.qrc)
...
/** Class encapsulating ULMEX Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (ULMEXGUI)
...
{
    window = new ULMEXGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ULMEXGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ULMEXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(ULMEXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ULMEXGUI::DEFAULT_WALLET);
        ...
        // ULMEX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ULMEXGUI::handlePaymentRequest);
        connect(window, &ULMEXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. ULMEX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ulmex_locale);
    Q_INIT_RESOURCE(ulmex);
    ...
    /// 6. Determine availability of data directory and parse ulmex.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", ULMEX_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // ulmex: links repeatedly have their payment requests routed to this process:
