...
#include "qt/ulmex/dashboardwidget.h"
#include "qt/ulmex/forms/ui_dashboardwidget.h"
#include "qt/ulmex/sendconfirmdialog.h"
#include "qt/ulmex/txrow.h"
...
DashboardWidget::DashboardWidget(ULMEXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareUlme, "square-chart-ulme");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelUlme, "text-chart-ulme");
    ...
    setCssProperty(ui->labelAmountUlme, "text-stake-ulme-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ULMEXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ULME")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isUlme = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ULME, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ulme = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalUlme += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesUlme.append(ulme);
        chartData->valuesMN.append(mn);

        int max = std::max(ulme, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesUlme);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalUlme > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountUlme, "text-stake-ulme");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountUlme, "text-stake-ulme-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountUlme, ui->labelAmountMN});
    ui->labelAmountUlme->setText(GUIUtil::formatBalance(chartData->totalUlme, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZULME) |
