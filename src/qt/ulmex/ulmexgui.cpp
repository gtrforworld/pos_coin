...
#include "qt/ulmex/qtutils.h"
#include "qt/ulmex/defaultdialog.h"
#include "shutdown.h"
...
const QString ULMEXGUI::DEFAULT_WALLET = "~Default";

ULMEXGUI::ULMEXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void ULMEXGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &ULMEXGUI::toggleHidden);
...
void ULMEXGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &ULMEXGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &ULMEXGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &ULMEXGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &ULMEXGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &ULMEXGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &ULMEXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &ULMEXGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &ULMEXGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &ULMEXGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &ULMEXGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &ULMEXGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &ULMEXGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &ULMEXGUI::execDialog);
}


void ULMEXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void ULMEXGUI::handleRestart(QStringList args)
{
...
void ULMEXGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &ULMEXGUI::message);
...
void ULMEXGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &ULMEXGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &ULMEXGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void ULMEXGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void ULMEXGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void ULMEXGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &ULMEXGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &ULMEXGUI::show);
                e->ignore();
            }
        }
    }
...
void ULMEXGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void ULMEXGUI::messageInfo(const QString& text)
{
...
void ULMEXGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool ULMEXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void ULMEXGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void ULMEXGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ULMEXGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void ULMEXGUI::goToSend()
{
    showTop(sendWidget);
}

void ULMEXGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void ULMEXGUI::goToMasterNodes()
{
    masterNodesWidget->resetCoinControl();
    showTop(masterNodesWidget);
}

void ULMEXGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void ULMEXGUI::goToGovernance()
{
    showTop(governancewidget);
}

void ULMEXGUI::goToSettings(){
    showTop(settingsWidget);
}

void ULMEXGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void ULMEXGUI::goToReceive()
{
    showTop(receiveWidget);
}

void ULMEXGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void ULMEXGUI::showTop(QWidget* view)
{
...
void ULMEXGUI::changeTheme(bool isLightTheme)
{
...
void ULMEXGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool ULMEXGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void ULMEXGUI::showHide(bool show)
{
...
int ULMEXGUI::getNavWidth()
{
    return this->navMenu->width();
}

void ULMEXGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void ULMEXGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void ULMEXGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool ULMEXGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &ULMEXGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &ULMEXGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &ULMEXGUI::message);
    connect(topBar, &TopBar::message, this, &ULMEXGUI::message);
    connect(sendWidget, &SendWidget::message,this, &ULMEXGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &ULMEXGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &ULMEXGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &ULMEXGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &ULMEXGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &ULMEXGUI::incomingTransaction);

    return true;
}

bool ULMEXGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void ULMEXGUI::removeAllWallets()
{
    // Single wallet supported.
}

void ULMEXGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(ULMEXGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ULMEXGUI::subscribeToCoreSignals()
{
...
void ULMEXGUI::unsubscribeFromCoreSignals()
{
