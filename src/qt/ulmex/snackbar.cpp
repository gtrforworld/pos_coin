...
#include "qt/ulmex/forms/ui_snackbar.h"
#include "qt/ulmex/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ULMEXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ULMEXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
