...
#include "config/ulmex-config.h"
#endif

#include "qt/ulmex/settings/settingsfaqwidget.h"
#include "qt/ulmex/settings/forms/ui_settingsfaqwidget.h"
#include "qt/ulmex/mnmodel.h"
#include "qt/ulmex/qtutils.h"

#include <QScrollBar>
#include <QMetaObject>

SettingsFaqWidget::SettingsFaqWidget(ULMEXGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableULME,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableULME,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableULME,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("ULMEX is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. ULMEX incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("ULMEX utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of ULMEX to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableULMEContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received ULMEX requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your ULMEX wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableULME->setText(unspendableULMEContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of ULMEX with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("ulmex-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.ULMEX.org'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::ULME)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in ULMEX Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the ULMEX network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableULME, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableULME);});
    ...
    if (parent)
        connect(parent, &ULMEXGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
