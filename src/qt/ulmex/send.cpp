...
#include "qt/ulmex/addnewcontactdialog.h"
#include "qt/ulmex/forms/ui_send.h"
#include "qt/ulmex/guitransactionsutils.h"
#include "qt/ulmex/loadingdialog.h"
#include "qt/ulmex/optionbutton.h"
#include "qt/ulmex/qtutils.h"
#include "qt/ulmex/sendchangeaddressdialog.h"
...
SendWidget::SendWidget(ULMEXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a ULMEX URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-ulme");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onULMESelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onULMESelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent ULMEs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield ULMEs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent ULMEs to shield."));
    }
}
...
void SendWidget::onULMESelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto ulmeAdd = Standard::DecodeDestination(address.toStdString(), isStaking, isExchange, isShielded);

        if (!Standard::IsValidDestination(ulmeAdd) || isStaking) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(ulmeAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ulmeAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
