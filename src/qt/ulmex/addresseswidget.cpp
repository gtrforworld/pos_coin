...
#include "qt/ulmex/forms/ui_addresseswidget.h"
#include "qt/ulmex/addressfilterproxymodel.h"
#include "qt/ulmex/addresslabelrow.h"
#include "qt/ulmex/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(ULMEXGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto ulmeAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStaking, isExchange, isShield);

        if (!Standard::IsValidDestination(ulmeAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(ulmeAdd);
        if (walletModel->updateAddressBookLabels(ulmeAdd, label.toUtf8().constData(),
