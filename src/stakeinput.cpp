// Copyright (c) 2017-2022 The ULMEX Core developers
...
CUlmeStake* CUlmeStake::NewUlmeStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CUlmeStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CUlmeStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CUlmeStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CUlmeStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CUlmeStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CUlmeStake::GetValue() const
{
    return outputFrom.nValue;
}

CDataStream CUlmeStake::GetUniqueness() const
{
    //The unique identifier for a ULME stake is the outpoint
...
const CBlockIndex* CUlmeStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CUlmeStake: uninitialized pindexFrom");
    return pindexFrom;
}
