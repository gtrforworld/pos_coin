...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("ULMEXMiner : generated block is stale");
    }
    ...
    if (!res || sc.get().stateErrorFound()) {
        return error("ULMEXMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ULMEXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ulmex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ULMEXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ULMEXMiner exception");
    } catch (...) {
        LogPrintf("ULMEXMiner exception");
    }

    LogPrintf("ULMEXMiner exiting\n");
}
