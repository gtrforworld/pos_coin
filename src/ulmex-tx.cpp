...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " ulmex-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  ulmex-tx [options] <hex-tx> [commands]  Update hex-encoded ulmex transaction\n" +
                               "or:     ulmex-tx [options] -create [commands]   Create hex-encoded ulmex transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ulmex transaction
