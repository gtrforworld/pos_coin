...
const char * const ULMEX_CONF_FILENAME = "ulmex.conf";
const char * const ULMEX_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ulmexd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ULMEX
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ULMEX
// Mac: ~/Library/Application Support/ULMEX
// Unix: ~/.ulmex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ULMEX";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ULMEXParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ULMEXParams
    // Mac: ~/Library/Application Support/ULMEXParams
    // Unix: ~/.ulmex-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ULMEXParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == nullptr || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "ULMEXParams";
#else
    // Unix
    return pathRet / ".ulmex-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/ulmex/sapling-spend.params";
        sapling_output = "/usr/share/ulmex/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/ulmex/sapling-spend.params";
            sapling_output = "/usr/local/share/ulmex/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", ULMEX_MASTERNODE_CONF_FILENAME));
