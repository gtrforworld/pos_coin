...
class ReorgStakeTest(UlmexTestFramework):
    ...
    def check_money_supply(self, expected_ulme):
        # verify that nodes have the expected ULME supply
        ulme_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(ulme_supply, [DecimalAmt(expected_ulme)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that ULME supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check ULME supply...")
