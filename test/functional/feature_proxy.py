...
"""Test ulmexd with different proxy configuration.

Test plan:
- Start ulmexd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on ulmexd side:
...
- Create ulmexds that connect to them
- Manipulate the ulmexds using addnode (onetry) an observe effects
...
class ProxyTest(UlmexTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: ulmexd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
