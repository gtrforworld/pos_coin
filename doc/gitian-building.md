...
*Setup instructions for a gitian build of ULMEX Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ULMEX
...
to the ULMEX GitHub release page.
...
- [Building ULMEX Core](#building-ulmex-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ulmex-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ulmex-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ulmex-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ulmex-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ulmex-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ulmex-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ulmex-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ulmex.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ulmex-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ulmex-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ulmex-osx-*-build.assert
```
