UNIX BUILD NOTES
====================
Some notes on how to build ULMEX Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile ULMEX Core and the dependencies,
...
This will build ulmex-qt as well, if the dependencies are met.
...
memory available when compiling ULMEX Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~ulmex/+archive/berkeley-db4).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:ulmex/berkeley-db4
    sudo apt-get update
...
To build ULMEX Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build ulmex-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ulmex-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip ulmexd" to strip the debug
...
Security
--------
To help make your ULMEX Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, ULMEX Core should be built with a non-executable stack
