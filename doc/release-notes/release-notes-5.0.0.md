ULMEX Core version *v5.0.0* is now available from:  <https://github.com/ulmex-project/ulmex/releases>
...
ULMEX Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, ULMEX Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run ULMEX v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving ULMEs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://ulmex.org .
...
* Top bar showing shield ULME balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield ULME balance.
...
See the release notes from [v4.2.0](https://github.com/ULMEX-Project/ULMEX/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed ULME supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in ULME
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in ULME
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zULMEsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield ULME)
  The `moneysupply` JSON return entry is the total ULME supply (the sum of all transparent and shield ULME)
...
| `getaccountaddress` | Returned the current ULMEX address associated with an internal account | Account system removal |
| `setaccount` | Associate a ULMEX address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given ULMEX address | Account system removal |
| `getaddressesbyaccount` | Returned a list of ULMEX addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a ULMEX address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a ULMEX address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zULME balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zULME mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zULME mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zULME amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zULME to a ULME address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zULME mints to a ULME address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zULMEs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzulmeseed` | Set the wallet's deterministic zULME seed to a specific value | Zerocoin deprecation |
| `getzulmeseed` | Return the current deterministic zULME seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zULME seed | Zerocoin deprecation |
| `dzulmestate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzulme` | Do a search for deterministically generated zULME that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_ulmex unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake ulmexd and unit tests: fixing missing MacOS framework. (furszy)
